 ...
 *    out of the last one, and — the part that matters — every one of them mixes
 *    toward `transparent` rather than toward an opaque colour, so the real
 *    surface is only knowable by compositing whatever panel happens to be
 *    behind. Fifteen of them carry text: the hero aside and `.trust-badge`
 *    (`--accent` at 6% and 7%), `.callout` and `.fork-node` (8%),
 *    `.indicator.primitive` (`--info-text` 8%), the two security verdicts
 *    (`--ok` 12% and `--danger` 16%), both `.sync-badge` tones (14%), both
 *    timeline `.blk` tones (12%), `.cracked` (16%), and the three byte
 *    highlights `.byte-same` / `.byte-chg` / `.byte-diff` (26%, 32%, 35%) that
 *    are how this lab shows a duplicated secret at all. axe files every one of
 *    them under `incomplete`, which means a violations-only gate measured the
 *    contrast of almost nothing this lab generates or concludes with.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. `style.css` declares no
 *    `opacity` at all, deliberately: its header says "Muted text lowers
 *    lightness, never opacity", and `.cl-hero-sub` carries an inline note that
 *    its colour "replaces opacity for AA-safe muting". The opacity handling
 *    below is therefore currently inert here, and is kept because that is a
 *    property of today's stylesheet rather than of the page — one `opacity: .7`
 *    on a muted note would silently move every ratio underneath it.
 *
 ...
 *    boxes are the two `.scroll-x` wrappers around Chapter 5's incident tables
 *    (`overflow: auto`, which both overflow at phone width), and `.progress` and
 *    `.ks-meter` (`overflow: hidden`, which clip their own fill bars). Content
 ...
 *    draws its figures — the fork() tree, the reseed timeline, the keyspace
 *    meter — in HTML and CSS flex/grid rather than SVG, so the sibling walk
 *    finds nothing to do beyond the shared top bar's two `aria-hidden` glyphs.
 ...
 *    than onto the `<rect>` drawn under it, and report a ratio nothing on screen
 *    has.
 *
 ...
 * `::before`/`::after`/`::marker` is not one — nor is it one to axe's
 * `color-contrast` rule. This page has exactly one, and it carries meaning:
 * `details.disclosure > summary::marker { color: var(--accent-text) }`. Unlike
 * most labs in this fleet the UA's own disclosure triangle is KEPT here rather
 * than replaced (no `list-style: none`, no `::-webkit-details-marker` override)
 * and merely recoloured, so that marker is the whole open/shut affordance on all
 * four disclosures. Measured by hand from real screenshot pixels, painted on the
 * `--panel-2` fill of `details.disclosure`: see the note in `gate.ts` beside
 * the drive step that opens them.
 *
 * SECOND, the guided-tour narration bar. `.tour-bar` is `position: fixed` at the
 * bottom of the viewport with `z-index: 30`, painted OVER whatever page content
 * happens to be under it. It is not an ancestor of that content, so this walk
 * does not see the occlusion and neither does axe.
 *
 * That is bounded rather than waved away, and in the harmless direction: the bar
 * paints an OPAQUE `background-color: var(--panel)`, so nothing shows through
 * it. Text underneath is hidden, not tinted — the ratios this walk reports for
 * it are the ratios it has when the bar is not there, which is the state it is
 * in for all but the tour steps, and the bar's own text is measured for real
 * against its own opaque fill. No reported ratio is off by any amount.
 */
 ...
 * uses. `style.css` declares no `transition`, no `animation` and no
 * `@keyframes` at all, so there is no half-painted intermediate frame for a
 * scan to land inside; what IS timed here lives in JavaScript (the 550ms
 * per-step delay in the clone and fork auto-runs, and the tour's dwells), and
 * the gate drives those to zero by asking for reduced motion, which those three
 * call sites read directly.
 *
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: fifteen text-bearing
     * surfaces on this page are authored with `color-mix(in oklab, … ,
     * transparent)`, and Chromium reports that to `getComputedStyle` unchanged
     * rather than converting it to sRGB. A regex that only understands
     * `rgb()/rgba()` sees `null` for every one of them and the walk then falls
     * through to the wrong backdrop — which fabricates failures (dark-on-light
     * read as dark-on-dark) and, far worse, could hide a real one. The 2D canvas is the browser's own colour
     ...
     * moment a gradient runs further than the text it is under. So each gradient
     * is *sampled* at the text's real location instead: linear by projecting
     ...
     *
     * This page has one gradient, `.ks-meter > span`'s
     * `linear-gradient(90deg, var(--accent), var(--accent-text))` — the keyspace
     * collapse bar. It carries no text today, so the linear branch resolves it
     * and finds nothing to report. It is measured rather than skipped because
     * the bar is the panel's headline graphic and a label placed on it is the
     * obvious next edit, and because `background-image` on `select` (a
     * `url()` data-URI chevron) is exactly the non-gradient layer the guard at
     * the top of this function exists to reject rather than mis-parse.
     */
     ...
     *
     * A driven pass here walks five chapters, and the expensive part is that
     * `compareHexBlock` renders ONE `<span>` PER BYTE PAIR: a single 32-byte key
     * comparison is 32 sibling spans, the fork panel renders three of those per
     * step across two steps, and the clone panel four. A late state carries
     * several hundred such spans, all re-walking the same ancestors up to
     * `<body>`. Without the caches the pass re-reads the same computed styles
     * and rects tens of thousands of times, once per state in a drive with more
     * than twenty of them. Nothing mutates the DOM during the pass, so the
     ...
     * is the two `.scroll-x` wrappers around Chapter 5's incident tables, which
     * overflow at phone width, plus `.progress` and `.ks-meter`
     * (`overflow: hidden` around their fill bars). `.visually-hidden` is handled
     * separately below.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details class="disclosure">`: two ship with the page (the jargon
      // primer in the intro and the framing note in Chapter 5) and two more are
      // rendered into a verdict once a run produces one. The gate opens each by
      ...
      // not merely inherited. `style.css` defines `.visually-hidden` as
      // `position:absolute; width:1px; height:1px; overflow:hidden;
      // clip:rect(0,0,0,0)`, and `consequenceStrip()` puts one inside EVERY
      // cause-to-consequence item — `<span class="visually-hidden"> causes
      // </span>` — as the spoken replacement for the `aria-hidden` "→" beside
      // it. Six of those render at first paint and they are present in every
      // state this gate scans. Each has a 1x1 rect and passes
      // `checkVisibility()` while painting exactly zero pixels, so measuring
      // them reports ratios for ink that was never laid down, in every single
     ...
     *
     * What this lab hides was enumerated one by one, and all of it is either a
     * glyph whose words sit immediately beside it in the same ink on the same
     * surface — the `✓`/`✗` in every `indicatorPair`, the `⚠`/`✓` opening each
     * `.blk-tag`, the `🩺` on the health monitor, the `✓` in each
     * `.trust-badge`, the machine glyphs `🖥`/`🌱`/`🧬`/`🎲` — or a pure
     * decoration with no text at all: the `.fork-split` connector drawn between
     * the parent and its children, the `.tour-dots` progress pips, and the `→`
     * in each `.cons-item`, which is the one case where the replacement is
     * explicit rather than incidental (a `.visually-hidden` " causes " sits
     * beside it).
     *
     * ONE THING ON THIS PAGE DID hide a VALUE, and it was fixed rather than
     * documented: `.cracked-tag` carried `aria-hidden="true"` around the whole
     * string `"🔓 SEED CRACKED"` — the headline of Chapter 3's result, and the
     * only place the word "cracked" appears — when the intent was plainly to
     * hide the padlock. Now only the glyph is hidden and the words are exposed,
     * so both this walk and axe measure them. The remaining live ticker,
     * `.ticker`, is still `aria-hidden`, correctly: it rewrites itself several
     * times a second with the boot-time and PID currently being tried, which is
     * not announceable, and the count it illustrates is carried by the
     * `role="status"` line directly above it. Its ratio is therefore measured by
     * hand instead — see `gate.ts`.
     */
     ...
     * flex/grid. The guard stays because `ui/dom.ts`'s `el()` helper supports an
     * `html` attribute that assigns `innerHTML`, which is the usual way the
     * mistake arrives: interpolating a string ARRAY into a template literal
     ...
     *
     * Here it currently resolves in the harmless direction: `style.css` gives
     * `html` an opaque `background-color: var(--bg)` (longhand, with a comment
     * saying it is there for the a11y gate) and declares no `height` on `html`
     * or `body`, so the root box already spans the document and the ancestor
     * walk would reach the same colour anyway. The guard is what keeps that true
     * if either fact changes. This page is five chapters and many screens deep
     * in both themes, so the day a `height: 100%` appears on `html` or `body`,
     * everything below the first screen would report against a white page that
     * does not exist — in a dark theme whose surfaces are all near-black brown.
     */
